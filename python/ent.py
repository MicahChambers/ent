...
class Ent:
    ...
    def __init__(self, entfile = None, md5state = None):
        """ Ent Constructor """
        self.error = 0
        self.files = dict()
        self.variables = {'.PWD' : os.getcwd()}
        self.jobs = list()
        self.md5state = md5state

        # load the file
        if entfile:
            self.load(entfile, md5state)

    def load(self, entfile, md5state):
        # Update MD5 State File
        if md5state:
            self.md5state = md5state 

        self.generators, self.variables = parseV1(entfile)
        if not self.generators or not self.variables:
